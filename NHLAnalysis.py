...
import matplotlib.dates as mdates# Load data
...
schedule_path = 'data/NHL_Schedule.csv'skaters_df = pd.read_csv(skaters_path, parse_dates=['gameDate'], dayfirst=False)
goalies_df = pd.read_csv(goalies_path, parse_dates=['gameDate'], dayfirst=False)# App Title
st.title("Hockey Data Viewer with Pie and Time-Series Charts")
st.write("Data from [Hockey Reference](https://www.hockey-reference.com/)")# Sidebar: select position
position = st.sidebar.radio("Select Player Position", ['Skater', 'Goalie'])if position == 'Skater':
    df = skaters_df
    ...
    stat_names = ['Shots Against', 'Goals Against', 'Saves']# Sidebar: player and stat selection
...
selected_stat = stats[stat_names.index(selected_stat_display)]# Sidebar: date filter
...
end_date = pd.to_datetime(st.sidebar.date_input("End Date", max_value=max_date, value=max_date))# Filter data
...
player_df = player_df.dropna(subset=[selected_stat])# Histogram threshold
